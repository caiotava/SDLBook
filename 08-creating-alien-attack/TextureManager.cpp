...
void TextureManager::drawFrame(std::string id, int x, int y, int width, int height, int currentRow, int currentFrame,
                               SDL_Renderer *renderer, double angle, int alpha, SDL_RendererFlip flip) {
    SDL_Rect srcRect;
    SDL_Rect destRect;
    ...
    SDL_SetTextureAlphaMod(textures[id], alpha);

    SDL_RenderCopyEx(renderer, textures[id], &srcRect, &destRect, angle, 0, flip);
}
...
void TextureManager::drawTile(std::string id, int margin, int spacing, int x, int y, int width, int height,
    ...
    SDL_RenderCopyEx(renderer, textures[id], &srcRect, &destRect, 0, 0, SDL_FLIP_NONE);
}
